...
This router exposes the product layers that were added after the original app:
- public source graph
- free product integrations
- partner endpoint plans
- MicroOverWorker bundles
- tranches
- Sentence-as-a-Service
- Info Gauntlets
- language-fi
- provider catalog

It is intentionally safe-by-default: live provider calls are not executed here.
Routes create plans, catalogs, and ProofBook-audited records that workers can
execute later once provider credentials and queues are configured.
"""
...
def build_product_router(db_factory: Callable[[], sqlite3.Connection]) -> APIRouter:
    ...
    def provider_requirements():
        return get_provider_requirements()

    @router.get("/providers/web3")
    def providers_web3():
        return get_web3_status()
    ...
    def providers_google():
        return get_google_status()

    @router.get("/data/value-density")
    def data_value_density():
        return value_density_report()

    @router.get("/data/high-density")
    def data_high_density():
        return {"providers": high_density_providers()}

    @router.get("/data/bundles")
    def data_bundles():
        return {"bundles": bundle_definitions()}

    @router.get("/public-sources")
    def public_sources():
        return public_source_expansion_report()

    @router.get("/public-sources/categories")
    def public_sources_categories():
        return sources_by_category()

    @router.get("/public-sources/top")
    def public_sources_top():
        return {"sources": top_public_sources()}

    @router.post("/public-sources/enrichment-plan")
    def public_sources_plan(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_public_source_enrichment_plan(
                conn,
                context=context(payload),
                listing=payload.get("listing", {}),
                bundle_id=payload.get("bundle_id", "all_public_local_underwriting"),
            )
    ...
    def free_products_stack():
        return recommended_stack()

    @router.post("/free-products/plan")
    def free_products_plan(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_free_product_integration_plan(
                conn,
                context=context(payload),
                listing=payload.get("listing", {}),
                objective=payload.get("objective", "upgrade MEMBRA backend into a 10x free-tier product stack"),
            )
    ...
    def partners_plan(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_partner_endpoint_plan(
                conn,
                context=context(payload),
    ...
    def microoverworker():
        return microoverworker_status()

    @router.post("/microoverworker/bundle")
    def microoverworker_bundle(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_microoverworker_product_bundle(
                conn,
                context=context(payload),
                listing=payload.get("listing", {}),
                wallet_payloads=payload.get("wallets", []),
    ...
    def language_fi():
        return language_fi_status()

    @router.post("/language-fi/localize")
    def language_fi_localize(payload: dict[str, Any]):
        return localize_listing_stub(payload.get("listing", {}), locale=payload.get("locale", "fi"))

    @router.get("/tranches")
    def tranches():
        return tranche_catalog()

    @router.post("/tranches/intent")
    def tranches_intent(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_tranche_intent(
                conn,
                context=context(payload),
                tranche_id=payload["tranche_id"],
                buyer_id=payload.get("buyer_id", "buyer_default"),
                wallet_payload=payload.get("wallet"),
                requested_locale=payload.get("locale", "en"),
                notes=payload.get("notes", ""),
            )

    @router.post("/tranches/approve")
    def tranches_approve(payload: dict[str, Any], x_admin_token: str | None = Header(default=None)):
        if payload.get("require_admin_token") and not x_admin_token:
            raise HTTPException(401, "Admin token required by request policy")
        with db_factory() as conn:
            ctx = context(payload)
            entitlement = build_entitlement_record(payload["intent"], approved_by=payload.get("approved_by", ctx.actor_id))
            event = record_entitlement(conn, ctx, entitlement)
            return {"success": True, "entitlement": entitlement, "proofbook_event": event}

    @router.post("/sentence-as-service/create")
    def sentence_create(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_sentence_as_service(
                conn,
                context=context(payload),
                listing=payload.get("listing", {}),
                tranche_id=payload.get("tranche_id", "lf_operator"),
                buyer_id=payload.get("buyer_id", "buyer_default"),
                wallet_payloads=payload.get("wallets", []),
    ...
    def info_bits():
        return {"info_bits": list_info_bits()}

    @router.post("/info-gauntlets/create")
    def info_gauntlets_create(payload: dict[str, Any]):
        with db_factory() as conn:
            return create_info_gauntlet_service(
                conn,
                context=context(payload),
                listing=payload.get("listing", {}),
                categories=payload.get("categories"),
                locale=payload.get("locale", "en"),
                objective=payload.get("objective", "package a high-density MEMBRA info gauntlet"),
            )
